...
更头疼的是，HotSpot 内部发生的事几乎全是内部机制：你写的 Java 先被编译成一种中间代码「字节码」，运行时再由 JIT 这个即时编译器把跑得多的热点代码现场翻译成机器码——可这个过程中，为什么某个方法没被内联（JIT 把方法直接展开省掉调用开销）？`Math.sin`、`Arrays.equals` 为什么能这么快（背后是一批被自动替换的标准库方法）？除此之外，JVM 还要在后台默默做垃圾回收（GC）——自动清理不再使用的内存，不用你手动释放；
