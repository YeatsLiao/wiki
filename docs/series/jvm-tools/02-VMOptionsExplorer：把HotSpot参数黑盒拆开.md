...
接手一个老服务，启动脚本里有 `-XX:+AggressiveOpts`、`-XX:MaxPermSize=256m`。你隐约记得 AggressiveOpts「过时了」，但不确从哪个版本；MaxPermSize 在新版上压根不认。搜索引擎给一堆矛盾答案。

VM Options Explorer 就是一张权威表——每个字段来自 OpenJDK 源码自动解析：「这个参数：JDK 6 引入、JDK 11 废弃、JDK 12 移除、类型 bool、默认 false」。
