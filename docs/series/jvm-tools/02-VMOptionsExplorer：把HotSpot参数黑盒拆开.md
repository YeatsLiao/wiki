...
这时候你需要的是一张能直接告诉你「这个参数：从 JDK 6 引入、JDK 11 废弃、JDK 12 移除、类型是 bool、默认 false、定义在 `share/runtime/globals.hpp`」的权威表格。VM Options Explorer 就是这张表——每个字段来自 OpenJDK 源码自动解析。
