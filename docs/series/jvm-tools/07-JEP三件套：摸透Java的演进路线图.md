# JVM工具解读 · JEP 三件套：摸透 Java 的演进路线图
...
你想搞懂「虚拟线程到底落地到哪一步了」，去 openjdk.org/jeps 翻——几百条 JEP 平铺在一页，按编号排，根本看不出「Loom 项目有哪些、各自什么状态」。

JEP 是 Java 新功能的提案单元，OpenJDK 把它们归到项目里——Loom 管并发（虚拟线程）、Valhalla 管值类型、Panama 管原生互操作、Amber 管语言小特性。Chris 的 JEP 三件套（JEPMap / JEPSearch / FullJEP）给这份路线图做了三种视角的索引。
...
JEP 三件套让你看见「Java 往哪走」。但 JVM 里还有一种「看不见的优化」——你写的方法，HotSpot 压根没执行你写的字节码，而是自动替换为了手写汇编。下一篇 VM Intrinsics Explorer（八）揭这个谜。
