...
「我这个 JDK 能用 ZGC 吗？」「升到 17，CMS 是不是没了？」「Corretto 和 Zulu 在 GC 支持上一样吗？」这些答案散落在各发行版文档里，记不住也搜不全。

> **注：** GC 是 JVM 自动回收不用的内存的机制。G1、ZGC、Shenandoah、CMS、Parallel、Serial、Epsilon 是几种不同的垃圾回收器——有的吞吐高、有的延迟低、有的只分配不回收。

GC Explorer 直接把可用性做成一张矩阵：行是 JDK 版本/发行版，列是 GC，单元格是「支持/不支持」。

打开 https://chriswhocodes.com/gc-explorer.html：
